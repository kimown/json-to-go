...
function jsonToGo(json, typename, flatten = true, example = false, allOmitempty = false)
{
    ...
    function parseStruct(depth, innerTabs, scope, omitempty)
    {
        ...
        if (flatten && depth >= 2)
        {
            ...
            for (let i in keys)
            {
                ...
                indenter(innerTabs)
                const typename = typeof scope[keys[i]] !== "object" ? format(keyname) : uniqueTypeName(format(keyname), seenTypeNames)
                typeof scope[keys[i]] === "object" && seenTypeNames.push(typename)
    ...
    function uniqueTypeName(name, seen) {
        ...
        let i = 0;
        while (true) {
            let newName = name + '__' + i.toString();
